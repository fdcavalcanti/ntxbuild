...
def test_setup_nuttx(builder):
    ...
    subprocess.run(["git", "clean", "-xfd"], cwd=builder.nuttx_path, 
...
def test_build_success(builder):
    """Test that nuttx.bin exists in the nuttx directory after build (recursive search)."""
    files = subprocess.run(["git", "clean", "-xfdn"], cwd=builder.nuttx_path, 
                           check=True, capture_output=True,  text=True)
