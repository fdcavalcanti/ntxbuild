...
from .utils import NUTTX_DEFAULT_DIR_NAME, find_nuttx_root
...
def prepare_env(
    ...
    if start:
        ...
        nuttxspace, apps_dir = find_nuttx_root(current_dir)

        create_base_env_file(nuttxspace, apps_dir, build_tool)
...
def get_builder():
    ...
    Loads the environment configuration and creates an appropriate builder
    (MakeBuilder or CMakeBuilder) based on the configured build tool.

    Returns:
        MakeBuilder or CMakeBuilder: A builder instance configured for
    ...
    return nuttx_builder(nuttxspace_path, apps_dir, build_tool)
...
def start(store_nxtmpdir, use_cmake, board, defconfig):
    ...
    Args:
        ...
        use_cmake: If True, use CMake instead of Make. Defaults to False.
    ...
    env = prepare_env(True, build_tool)

    # Run NuttX setup using the builder (includes validation)
    click.echo("\n🔧 Setting up NuttX configuration...")
    click.echo(f"   NuttX directory: {env.get('nuttx_dir')}")
    click.echo(f"   Apps directory: {env.get('apps_dir')}")

    click.echo(f"   Build tool: {env.get('build_tool')}\n")
...
def make(ctx):
    ...
    The make command and its arguments should be provided as additional
    arguments after the 'make' command, e.g.:
        ntxbuild make all
        ntxbuild make clean
        ntxbuild make menuconfig
...
def cmake(ctx):
    ...
    The cmake command and its arguments should be provided as additional
    arguments after the 'cmake' command, e.g.:
        ntxbuild cmake --build build
        ntxbuild cmake -B build

    Exits with code 0 on success, 1 on error, or the cmake command's
    exit code on failure.
    """
    command = " ".join(tuple(ctx.args))
...
def boards(soc):
    ...
    # available boards.
    nuttxspace, _ = find_nuttx_root(current_dir)

    try:
        nuttx_path = nuttxspace / NUTTX_DEFAULT_DIR_NAME
...
def defconfigs(board, read):
    ...
    """
    current_dir = Path.cwd()
    logger.debug(f"Search for '{board}' defconfigs")
    ...
    try:
        nuttx_path = nuttxspace / NUTTX_DEFAULT_DIR_NAME
        explorer = NuttxBoardExplorer(nuttx_path)
        logger.debug(f"Explorer: {explorer}")
