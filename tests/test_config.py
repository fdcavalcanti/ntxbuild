...
def test_config_read_write_str(config, nuttxspace_path):
    config_manager = ConfigManager(nuttxspace_path, NUTTX_APPS_DIR)
...
def test_merge_config(nuttxspace_path):
    ...
    new_config_manager = ConfigManager(nuttxspace_path, NUTTX_APPS_DIR)
...
def test_config_manager_file_not_found_error(nuttxspace_path):
    ...
    with pytest.raises(FileNotFoundError, match="Apps path not found at"):
        ConfigManager(nuttxspace_path, apps_dir="nonexistent_apps")
...
def test_config_manager_runtime_error_no_config(nuttxspace_path, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="\\.config file not found"):
        ConfigManager(tmp_path, NUTTX_APPS_DIR)
